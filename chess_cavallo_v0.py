...
def solve_puzzle(matrice, start_x, start_y, mov_count):
    ...
    for move in possible_movements:
        ...
        if is_next_move_valid(matrice, next_x, next_y):
            mosse_valide.append([next_x, next_y])

    if len(mosse_valide) > 0:

        random_arr = random.choice(mosse_valide)
...
def run():
    ...
    if conta_combianzioni % 10000 == 1:
        print("COMBINAZIONE => ", conta_combianzioni)
